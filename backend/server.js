...
app.post("/register", (req, res) => {
  try {
    ...
    const checkPhoneSQL = "SELECT phone FROM user_details WHERE phone = ?";
    const insertSQL =
      "INSERT INTO user_details(`name`,`phone`,`email`,`gender`,`password`,`is_admin`,`is_active`) VALUES (?)";
...
app.post("/login", (req, res) => {
  try {
    ...
    const sql =
      "SELECT * FROM user_details WHERE phone = ? AND is_active = 1";
...
app.get("/getAllUsers", (req, res) => {
  try {
    const sql =
      "SELECT user_id, name, phone, email, gender, is_active FROM user_details WHERE is_admin = 0 AND is_active = 1";
...
app.put("/adminEditUser", (req, res) => {
  try {
    ...
    const sql =
      "UPDATE user_details SET name=?, phone=?, email=?, gender=? WHERE user_id = ?";
    const values = [
      name,
      phone,
      email,
...
app.put("/adminDeleteUser/:user_id", (req, res) => {
  try {
    ...
    const updateEmployeeDetailsSQL =
      "UPDATE user_details SET is_active = 0 WHERE user_id = ?";
    ...
    db.query(updateEmployeeDetailsSQL, [user_id], (err, result) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Error updating user_details" });
      }
